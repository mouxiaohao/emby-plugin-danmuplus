#!/usr/bin/env python3
"""Atomically migrate only the unique deployed V22 smart-match entry to V23."""
...
OLD_MARKER = "__embyDanmuSmartMenuV22"
NEW_MARKER = "__embyDanmuSmartMenuV23"
...
def main() -> int:
    ...
    if config.count(OLD_MARKER) != 1 or NEW_MARKER in config:
        raise RuntimeError("expected exactly one deployed V22 entry and no V23 entry")
    if script.count(NEW_MARKER) != 1 or OLD_MARKER in script:
        raise RuntimeError("candidate script must contain exactly one V23 marker and no V22 marker")
    ...
    if len(matching_entries) != 1 or matching_entries[0].group(2).count(OLD_MARKER) != 1:
        raise RuntimeError("V22 marker is not isolated inside exactly one content entry")
    ...
    if OLD_MARKER in updated or updated.count(NEW_MARKER) != 1:
        raise RuntimeError("updated configuration violates the V22-to-V23 migration contract")
