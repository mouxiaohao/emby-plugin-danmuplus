#!/usr/bin/env python3
"""Atomically migrate only the unique deployed V21 smart-match entry to V22."""
...
OLD_MARKER = "__embyDanmuSmartMenuV21"
NEW_MARKER = "__embyDanmuSmartMenuV22"
...
def main() -> int:
    ...
    if config.count(OLD_MARKER) != 1 or NEW_MARKER in config:
        raise RuntimeError("expected exactly one deployed V21 entry and no V22 entry")
    if script.count(NEW_MARKER) != 1 or OLD_MARKER in script:
        raise RuntimeError("candidate script must contain exactly one V22 marker and no V21 marker")
    ...
    if len(matching_entries) != 1 or matching_entries[0].group(2).count(OLD_MARKER) != 1:
        raise RuntimeError("V21 marker is not isolated inside exactly one content entry")
    ...
    if OLD_MARKER in updated or updated.count(NEW_MARKER) != 1:
        raise RuntimeError("updated configuration violates the V21-to-V22 migration contract")
