#!/usr/bin/env python3
"""Atomically migrate only the unique deployed V18 smart-match entry to V19."""
...
OLD_MARKER = "__embyDanmuSmartMenuV18"
NEW_MARKER = "__embyDanmuSmartMenuV19"
...
def main() -> int:
    ...
    if config.count(OLD_MARKER) != 1 or NEW_MARKER in config:
        raise RuntimeError("expected exactly one deployed V18 entry and no V19 entry")
    if script.count(NEW_MARKER) != 1 or OLD_MARKER in script:
        raise RuntimeError("candidate script must contain exactly one V19 marker and no V18 marker")
    ...
    if len(matching_entries) != 1 or matching_entries[0].group(2).count(OLD_MARKER) != 1:
        raise RuntimeError("V18 marker is not isolated inside exactly one content entry")
    ...
    if OLD_MARKER in updated or updated.count(NEW_MARKER) != 1:
        raise RuntimeError("updated configuration violates the V18-to-V19 migration contract")
