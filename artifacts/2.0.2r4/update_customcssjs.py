...
def main() -> int:
    ...
    if config.count("__embyDanmuSmartMenuV15") != 1:
        raise RuntimeError("expected exactly one deployed V15 marker")
    if "__embyDanmuSmartMenuV15" not in script:
        raise RuntimeError("candidate script does not contain V15 marker")
    ...
    if updated.count("__embyDanmuSmartMenuV15") != 1 or "__embyDanmuSmartMenuV14" in updated:
        raise RuntimeError("updated marker contract is invalid")

    temporary_path = config_path + ".r4-new"
