 ...
 * 适用于 Emby 4.9.x + 本方案配套的 Emby.Plugin.Danmu 2.0.3r1 DLL
 */
(function () {
    "use strict";

    // V17 adds safe re-match/remove handling for virtual seasons used when one Emby season contains
    // several upstream seasons or specials.
    var INSTALL_FLAG = "__embyDanmuSmartMenuV17";
    ...
    async function requestAuthoritativeCompositePlan(item, season, compactSelections) {
        ...
        var preview = await api(parameters.seriesId || item.Id, "MatchPreview", parameters);
    ...
    function appendCompositeMappingDetails(card, group) {
        if (group.kind !== "mapped" || !group.episodes.length) return;
        var details = document.createElement("details");
        var summary = document.createElement("summary");
        summary.textContent = "展开逐集精确映射（" + group.episodes.length + " 集）";
        details.appendChild(summary);
        group.episodes.forEach(function (episode, index) {
            var mapping = group.mappings[index] || episode;
            var line = document.createElement("div");
            line.className = "danmuVirtualSeasonDetail";
            var sourceNumber = value(mapping, "SourceEpisodeNumber", "sourceEpisodeNumber", null);
            line.textContent = localEpisodeLabel(episode) + " → " +
                (sourceNumber ? "来源第 " + sourceNumber + " 集" : "来源剧集已由服务器验证");
        ...
        groups.forEach(function (group, groupIndex) {
            ...
            if (group.kind === "unmatched") {
                ...
                remove.addEventListener("click", function () {
                    removeCompositeSelection(selections, season,
                        value(group.selection, "LocalStartEpisodeItemId", "localStartEpisodeItemId", ""));
                    renderSeriesPicker(dialog, item, seasons, selections, keywords);
                });
                actions.append(rematch, remove);
                card.appendChild(actions);
            } else if (group.kind === "mapped" && !isDirectCompositeOrigin(group.origin)) {
                var mappedActions = document.createElement("div");
                var mappedStart = value(group.episodes[0], "ItemId", "itemId", "");
                async function rebuildWithoutMappedGroup(reopenPicker) {
                    var compact = compositeRequestSelections(selections, season).filter(function (selection) {
                        return value(selection, "LocalStartEpisodeItemId", "localStartEpisodeItemId", "") !== mappedStart;
                    });
                    setBusy(dialog, reopenPicker ? "正在移除旧映射并准备重新匹配…" : "正在移除虚拟季映射…");
                    try {
                        var confirmed = await requestAuthoritativeCompositePlan(item, season, compact);
                        ...
                        if (reopenPicker) {
                            var run = compositeVirtualGroups(confirmed, selections).find(function (candidate) {
                                return candidate.kind === "unmatched" && candidate.episodes.some(function (episode) {
                                    return value(episode, "ItemId", "itemId", "") === mappedStart;
                                });
                            });
                            if (run) {
                ...
                mappedRematch.addEventListener("click", function () { rebuildWithoutMappedGroup(true); });
                var mappedRemove = document.createElement("button");
                mappedRemove.className = "danmuSmartButton";
                mappedRemove.textContent = "移除";
                mappedRemove.addEventListener("click", function () { rebuildWithoutMappedGroup(false); });
    ...
    function renderCompositeGroupPicker(dialog, item, season, seasonIndex, seasons, selections, keywords, group) {
        dialog.androidBackLocked = false;
        ...
        input.value = hasExplicitKeyword ? keywords[key] : value(season, "SeriesName", "seriesName", item.Name || "");
        ...
        async function searchCurrentGroup() {
            var keyword = input.value.trim();
            if (!keyword) {
                notify("请输入临时季的搜索关键词。", true);
                return;
            }
            setBusy(dialog, "正在搜索临时季候选…");
            try {
                var parameters = seasonRequestParameters(season);
                ...
                parameters.compositePlan = "true";
                var refreshed = await api(parameters.seriesId || item.Id, "MatchPreview", keywordRematchParameters(parameters, input));
        ...
        searchButton.addEventListener("click", searchCurrentGroup);
        input.addEventListener("keydown", function (event) {
            if (event.key === "Enter") { event.preventDefault(); searchCurrentGroup(); }
        });
        ...
        save.addEventListener("click", async function () {
            ...
            try {
                var confirmed = await requestAuthoritativeCompositePlan(
                    item, season, compositeRequestSelections(selections, season));
                seasons[seasonIndex] = confirmed;
                clearCompositeSelectionStore(selections, season);
                renderSeriesPicker(dialog, item, seasons, selections, keywords);
            } catch (error) {
    ...
    function renderSeriesPicker(dialog, item, seasons, selections, keywords) {
        ...
        seasons.forEach(function (season, seasonIndex) {
            ...
            manual.addEventListener("click", async function () {
                ...
                setBusy(dialog, "正在请求服务器重新智能匹配…");
                try {
                    var parameters = seasonRequestParameters(season);
                    var refreshed = await api(parameters.seriesId || item.Id, "MatchPreview", rematchParameters(parameters));
                    var refreshedSeason = (value(refreshed, "Seasons", "seasons", []) || [])[0];
    ...
    async function runSmartDownload(item, dialog) {
        setBusy(dialog, item.Type === "Series" ? "正在逐季请求服务器匹配结果，请稍候…" : "正在请求服务器匹配结果，请稍候…");
