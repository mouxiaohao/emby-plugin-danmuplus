#!/usr/bin/env python3
"""Atomically migrate only the unique deployed V17 smart-match entry to V18."""
...
OLD_MARKER = "__embyDanmuSmartMenuV17"
NEW_MARKER = "__embyDanmuSmartMenuV18"
...
def main() -> int:
    ...
    if config.count(OLD_MARKER) != 1 or NEW_MARKER in config:
        raise RuntimeError("expected exactly one deployed V17 entry and no V18 entry")
    if script.count(NEW_MARKER) != 1 or OLD_MARKER in script:
        raise RuntimeError("candidate script must contain exactly one V18 marker and no V17 marker")

    matching_entries = [
        match for match in CONTENT_PATTERN.finditer(config)
        if OLD_MARKER in match.group(2)
    ]
    if len(matching_entries) != 1 or matching_entries[0].group(2).count(OLD_MARKER) != 1:
        raise RuntimeError("V17 marker is not isolated inside exactly one content entry")

    replacement = html.escape(script, quote=False)
    entry = matching_entries[0]
    updated = config[:entry.start(2)] + replacement + config[entry.end(2):]
    if OLD_MARKER in updated or updated.count(NEW_MARKER) != 1:
        raise RuntimeError("updated configuration violates the V17-to-V18 migration contract")

    original = os.stat(config_path)
    config_directory = os.path.dirname(config_path)
    temporary_fd, temporary_path = tempfile.mkstemp(
        prefix=".danmu-customcssjs-",
        suffix=".tmp",
