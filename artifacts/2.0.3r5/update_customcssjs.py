#!/usr/bin/env python3
"""Atomically migrate only the unique deployed V20 smart-match entry to V21."""
...
OLD_MARKER = "__embyDanmuSmartMenuV20"
NEW_MARKER = "__embyDanmuSmartMenuV21"
...
def main() -> int:
    ...
    if config.count(OLD_MARKER) != 1 or NEW_MARKER in config:
        raise RuntimeError("expected exactly one deployed V20 entry and no V21 entry")
    if script.count(NEW_MARKER) != 1 or OLD_MARKER in script:
        raise RuntimeError("candidate script must contain exactly one V21 marker and no V20 marker")
    ...
    if OLD_MARKER in updated or updated.count(NEW_MARKER) != 1:
        raise RuntimeError("updated configuration violates the V20-to-V21 migration contract")
