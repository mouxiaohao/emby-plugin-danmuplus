...
namespace Emby.Plugin.Danmu.Scraper.Entity
{
    ...
    {
        /// <summary>
        /// 分段来源的总分段数。仅用于把下载质量传递给任务界面，不写入 XML。
        /// </summary>
        [XmlIgnore] public int SegmentTotal { get; set; }

        /// <summary>
        /// 已经重试后仍下载失败的分段数。其余分段仍会正常写入 XML。
        /// </summary>
        [XmlIgnore] public int SegmentFailed { get; set; }

        [XmlIgnore] public List<string> FailedSegmentNames { get; set; } = new List<string>();
