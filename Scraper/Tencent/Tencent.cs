...
namespace Emby.Plugin.Danmu.Scraper.Tencent
{
    public class Tencent : AbstractScraper
    {
        ...
        public override async Task<ScraperDanmaku?> GetDanmuContent(BaseItem item, string commentId)
        {
            ...
            var downloadResult = await _api.GetDanmuContentAsync(commentId, CancellationToken.None).ConfigureAwait(false);
            var danmaku = new ScraperDanmaku();
            danmaku.ChatId = 1000;
            danmaku.ChatServer = "dm.video.qq.com";
            danmaku.ProviderId = ProviderId;
            danmaku.SegmentTotal = downloadResult.SegmentTotal;
            danmaku.SegmentFailed = downloadResult.SegmentFailed;
            danmaku.FailedSegmentNames.AddRange(downloadResult.FailedSegmentNames);
            foreach (var comment in downloadResult.Comments)
            {
                try
                {
