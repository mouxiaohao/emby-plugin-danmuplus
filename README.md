...
- 移除季度冲突证据的 0.79 分上限和将 0.85 分符号保真候选抬到自动阈值的桥接；季度评分采用父剧名 60 分、季名 20 分、年份精确吻合 20 分，集数不再计分或阻断匹配，90 分普通自动阈值及真正同分时的保真决胜保持不变。
- 复合季度下载映射提示在适用结果中只显示一次；权威映射确认库内集数少于来源集数时，整剧和单季结果显示黄色提示，库内集数较多时仍按现有流程形成余集临时季。配置页标题为 `DanmuPlus 配置`，源码链接指向本项目 `main` 分支，插件程序集、标识、配置路由与已保存设置兼容性不变。
