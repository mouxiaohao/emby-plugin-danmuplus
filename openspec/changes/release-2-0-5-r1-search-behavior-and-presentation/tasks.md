...
- [x] 17.7 Run the affected focused suites, full backend/frontend regressions, sequential Release build, strict OpenSpec validation, diff/scope/credential checks, Sol-high final review, then back up and deploy the verified DLL/CustomCssJS pair under the existing authorization without initiating a download during live testing.

## 18. Post-slice strict full-title Season fallback

- [x] 18.1 Specify a same-pair strict complete-title Season-name fallback that runs only when both loose parent-stripped remainders are empty; preserve punctuation through NFKC fidelity equality, reject parent-only and conflicting-Season cases, take the maximum with ordinary Season evidence, and keep l9, Season 1, source-channel isolation, and Movie behavior unchanged.
- [x] 18.2 Implement the fallback in the Season scorer and add deterministic TitleFidelity coverage for `妄想学生会＊/*`, markerless and different-symbol candidates, one-sided/non-empty remainders, parent punctuation, parent-only titles, conflicting Season markers, source-channel isolation, maximum-not-addition, Season 1, l9, and Movie boundaries.
- [x] 18.3 Run TitleFidelity, TMDB alias, SearchTermPolicy, R3 search quality, complete backend regression, sequential Release build, strict OpenSpec validation, diff/scope/credential checks, and Sol-high final review; record the new independently revertible commit and artifact hash without claiming deployment.
- [x] 18.4 After explicit authorization for this new artifact, back up the deployed DLL/CustomCssJS/plugin-configuration three-file set, deploy and read back only the verified DLL, and live-preview `妄想学生会` Seasons 1 and 2 without initiating a download; verify the starred Season 2 candidate receives 100, the markerless 2010 candidate remains unpromoted at 60 against the 2014 Season, and the existing empty-residual Season 1 candidate remains 100.
