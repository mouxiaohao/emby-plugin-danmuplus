...
A short TMDB parent alias can leave the remainder of the provider's longer parent title in front of an otherwise exact Season marker. Recover that case only in the TMDB alias parent-maximum path and inside one source-title channel: every usable real target remainder must be generic, at least one must equal the expected Season label, the source remainder must contain one correct terminal Season marker, and any preceding text must contain at least four characters with at least one letter and reach `0.90` against a strictly equal-length window of a known parent title. The prefix is then treated as an unconsumed parent-title continuation solely for Season-name comparison; it never adds parent points. Named Seasons, ordinary search scoring, unrelated or overlong prefixes, trailing text, conflicting markers, cross-channel evidence, Movie scoring, and global title normalization remain on their existing paths.

Some official sequel identities are expressed only by punctuation, such as a library Season `妄想学生会＊` and source title `妄想学生会*`. Loose normalization intentionally removes those symbols, so both parent-stripped Season remainders become empty even though the complete titles are compatibility-equivalent. Add a fallback inside one concrete library-title/source-title/matched-parent comparison only after the ordinary Season remainder has failed to reach full evidence and both loose remainders are empty. Compare the two complete titles with the existing fidelity normalization: apply Unicode NFKC, case folding, and whitespace removal, but do not remove the parent, delete punctuation, rewrite Season markers, or use fuzzy similarity. A strict equality may supply the existing 20-point Season-name component only when the local complete Season title differs from every applicable complete parent title and no conflicting Season marker exists. The ordinary and fallback Season evidence take their maximum and never add together. This is ordinary Season-name evidence, not a fidelity confidence bonus; it does not restore l9, combine source title channels, change the Season 1 empty-remainder rule, or affect Movie scoring.
...
- [Strict full-title fallback could count a parent title twice or turn decorative punctuation into a broad bonus] → require both loose Season remainders to be empty, strict fidelity equality within the same matched-parent/source-title pair, a local complete Season title distinct from every applicable parent title, and maximum rather than additive Season evidence.
