...
The system SHALL use conservative title markers and duplicate-number evidence when structured metadata is absent or incomplete, and MUST NOT classify an episode as non-main based only on a small downloaded XML file size.
...
- **WHEN** an entry is short or produces a small danmu XML but is not otherwise identified as non-main content
- **THEN** the system SHALL NOT reject it solely for its duration or XML byte size
